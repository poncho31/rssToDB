...
// INSERT DANS ARTICLESCOMPLET 
$db = new Database();
$sql = "SELECT idMedia, lien, date, description, nom, titre FROM media WHERE nom <> 'levif' ORDER BY nom";
$stmt = $db->getQuery($sql);
$stmt->execute();
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
	...
	$medias = ['rtl', 'lesoir', 'rtbf', 'lecho', 'dh', 'lalibre', 'sudinfo'];
	if (in_array($row['nom'], $medias)) {
		$dataFromDB = ['idMedia' =>$row['idMedia'],
						'lien'=>$row['lien'], 
						'nomMedia'=>$row['nom'], 
						'titre'=>$row['titre'], 
		...
		$urlContent = file_get_contents($row['lien']);
		...
		for($i=0;$i<$Elements->length;$i++) {
			if($Elements->item($i)->attributes->getNamedItem('class')){
				if($Elements->item($i)->attributes->getNamedItem('class')->nodeValue == $mediaClass[$row['nom']]) {
		...
		$dataFromDB ['article'] = $json;
		$sqlInsert = "
				SET NAMES 'utf8'; 
				SET CHARACTER SET utf8;
				INSERT INTO articlescomplet
							(lien, nomMedia, titre, description, date, article, fk_idMedia)
				VALUES 
							('".$dataFromDB['lien']."', 
							'".$dataFromDB['nomMedia']."', 
							'".$dataFromDB['titre']."', 
		...
		$db2 = new Database();
		$db2->getQuery($sqlInsert);
	}
}

// function getElementsByClassName($dom, $ClassName, $tagName=null, $dataFromDB) {
//     if($tagName){
